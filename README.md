...
This step runs the YOLO model over the input footage frame by frame and writes detection results to `detections.jsonl`. Run metadata is written to `run_info.jsonl`.

### Tracking

To run object tracking using the detections generated during inference from the repository root:
```bash
python3 src/track.py --detections <PATH_TO_DETECTIONS> --output <PATH_TO_TRACKS>
```

| Argument | Description
| --- | --- |
| `--detections` | Path to detections records generated by the inference step
| `--output` | Path where tracking results will be written.

This step reads `detetcions.jsonl` and associates detections across frames using IoU-based tracking. Resulting object tracks are written to `tracks.jsonl`.
...
- Inference parameters (confidence interval, IoU threshold, image size, device, stride, save settings)
...
Each line in `tracks.jsonl` is a seperate JSON object.
For example:
```json
