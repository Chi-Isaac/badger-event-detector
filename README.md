...
| Argument | Description | Required | Default Value |
| --- | --- | --- | --- |
| `--detections` | Path to detections records generated by the inference step | Yes | N/A |
| `--output` | Path where tracking results will be written. | Yes | N/A |
| `--iou` | IoU threshold used to match detections to existing tracks (decimal from 0.0-1.0). | No | 0.3 |
